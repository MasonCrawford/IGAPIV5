...
public class AuthenticationResponse
{
    ...
    /// <Summary>
    ///     If specified, indicates that the authentication process requires the client to switch to a different URL in order
    ///     to complete the login.
    ///     If null, no rerouting has to take place and the authentication process is complete.
    ///     This is expected for any DEMO clients, where the authentication process is initiated against the production servers
    ///     (i.e. https://api.ig.com/gateway/deal )
    ///     whereas all subsequent requests have to be issued against the DEMO servers (i.e.
    ///     https://demo-api.ig.com/gateway/deal )
    ///     Please also note that when rerouting to DEMO it is also required to invoke to "silent login" endpoint in DEMO with
    ///     the CST token
